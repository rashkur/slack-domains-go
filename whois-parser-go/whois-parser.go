...
func Parse(text string) (wi WhoisInfo, err error) {
	...
	parseLen := len(text)
	parseLenStr := strconv.Itoa(parseLen)
	if parseLen < 100 {
		if IsNotFound(text) {
			return wi, errors.New("Domain is not found! Response length: " + parseLenStr + ". " + text)
		} else {
			//fmt.Println("invalid here! "+parseLenStr, text)
			return wi, errors.New("Domain whois data invalid! Response length:" + parseLenStr + ". " + text)
		}
	}
	...
	//wi.Registrar.DomainName = strings.ToLower(wi.Registrar.DomainName)
